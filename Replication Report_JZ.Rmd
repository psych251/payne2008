...
#### Actual Sample
  ...
  Fourty-five participants completed Session 1, and only 25 of them finished Session 2. Two more participants were excluded because they finished Session 2 within one hour after Session 1, leaving a final sample of 23 participants (`r percent(nrow(data_s1s2 %>% filter(gender == 'Female'))/nrow(data_s1s2))` female; age: `r mean(data_s1s2$age)` \pm `r sd(data_s1s2$age)`). `r percent(nrow(data_s1s2 %>% filter(!grepl("high school", education)))/nrow(data_s1s2))` of the sample has finished or attended college education. In the final sample, 14 were in the sleep condition, and 9 were in the wake condition.

#### Differences from pre-data collection methods plan
  
  Due to slow data collection speed on Mturk and high dropout rate, the final sample has only 23 participants, which is smaller than the planned sample size (*n* = 48).
...
data_s2_clean <- data_s2_clean %>% 
  mutate(time = mdy_hm(StartDate), condition = ifelse(hour(time) >= 5 & hour(time) <= 14, "sleep", "wake")) # considering time difference, the sleep group (across the US) starts Session 2 between 5 am and 2 pm 
...
# hist(hour(data_joined$time), breaks = 24, main = "Distribution of Session 2 start time")
...
data_ind <- data_joined %>% 
  ...
  group_by(code_s1, condition, component, valence, s2_q1) %>% 
  mutate(count_ans = n()) #the number of all answers separately (same, similar, new)
data_ind <- data_ind %>% 
  filter(type == "same") %>% 
  group_by(code_s1, condition, component, valence) %>% 
...
# mod0 <- lmer(general_recog ~ condition*component+component*valence+condition*valence + (component*valence | code_s1), data_ind)
# mod1 <- lmer(general_recog ~ condition*component*valence + (component*valence | code_s1), data_ind)
# anova(mod0, mod1)
...
anova_2way_obj = ezANOVA(
    data = data_ind %>% filter(component == "object"),
    dv = .(general_recog),
    wid = .(code_s1), 
    within = .(valence),
    between = .(condition)
...
# linear mixed effect models
# mod2 <- lmer(general_recog ~ condition + valence + (valence | code_s1), data_ind %>% filter(component == "object"))
# mod3 <- lmer(general_recog ~ condition*valence + (valence | code_s1), data_ind %>% filter(component == "object"))
# anova(mod2, mod3)
```
...
anova_2way_bgd = ezANOVA(
    data = data_ind %>% filter(component == "background"),
    dv = .(general_recog),
    wid = .(code_s1), 
    within = .(valence),
    between = .(condition)
...
# linear mixed effect mdoels
# mod4 <- lmer(general_recog ~ condition + valence + (valence | code_s1), data_ind %>% filter(component == "background"))
# mod5 <- lmer(general_recog ~ condition*valence + (valence | code_s1), data_ind %>% filter(component == "background"))
# anova(mod4, mod5)
```
...
# mod6 <- lmer(specific_recog ~ condition + valence + (1 | code_s1), data_ind %>% filter(component == "object"))
# mod7 <- lmer(specific_recog ~ condition*valence + (1 | code_s1), data_ind %>% filter(component == "object"))
# anova(mod6, mod7)
