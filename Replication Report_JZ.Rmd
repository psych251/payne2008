...
There are 2 attention-check questions in session 1 rating data. Participants who failed either attention check will be excluded from further analysis. Ratings of approach-withdraw towards 64 picture stimuli in session 1 will be gathered into a long form with a label of either neutral scene or negative scene. Ratings of neutral scenes and negative scenes will be compared as a manipulation check (participants are expected to have higher withdraw ratings towards negative scenes than neutral scenes).
...
####Import data
data_raw <- read_csv("data/PSYCH251+Session+2_November+20%2C+2018_11.54.csv")
...
data <- data %>% mutate(item_number = as.numeric(str_sub(item, 1, str_length(item)-3))) %>% arrange(ResponseId, item_number) 
...
item_list <- item_list %>% mutate(component = ifelse(item_number %in% c(1:64, 129:160), "object", "backgrond"))
# create valence (neutral or negative)
item_list <- item_list %>% mutate(valence = ifelse(item_number %in% c(A, B, E, F,  #object
...
data_ind <-  data_ind %>% 
  mutate(specific_recog = same/count_all, general_recog = similar/(count_all-same))
...
data_summary <- data_ind %>% 
  ...
  summarise(specific_recog = mean(specific_recog, ns.rm = T), general_recog = mean(general_recog, ns.rm = T), count = n())
...
```{r}
## test on the 2-way interaction
mod4 <- lmer(general_recog ~ condition + valence + (1 | ResponseId), data_ind %>% filter(component == "object"))
summary(mod4)
mod5 <- lmer(general_recog ~ condition*valence + (1 | ResponseId), data_ind %>% filter(component == "object"))
summary(mod5)
anova(mod4, mod5)
