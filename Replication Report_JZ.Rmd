...
```{r}
#Erin suggested analysis
m <- lm(general_recog ~ condition*component*valence, data_ind)
anova(m)
```
...
# this works though!
# sd(data_ind$specific_recog)
```

```{r}
# plot general recognition
p1 <- ggplot(data_summary , aes(x = condition, y = general_recog, fill = component))+
...
p2 <- ggplot(data_summary , aes(x = condition, y = specific_recog, fill = component))+
  geom_bar(color = "black", stat = "identity", position=position_dodge(), width=0.5) + 
  facet_grid(. ~ valence)+
