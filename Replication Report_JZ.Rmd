...
####Import data
#wake group data
data_raw_wake <- readSurvey("data/PSYCH251+Session+2_sleep.csv")
#sleep group data
data_raw_sleep <- readSurvey("data/PSYCH251+Session+2_wake.csv")
#combine the two raw datasets
data_raw <- rbind(data_raw_wake,data_raw_sleep)

#### Data exclusion / filtering
data <- data_raw %>% 
  select(-contains("Q34"), -contains("Q35"), -contains("Q31"), -starts_with("Recipient"), -contains("s2p")) %>% 
...
temp2 <- data %>% 
  select(ResponseId, contains("s2_q2")) %>% 
  gather(item, s2_q2, contains("s2_q2"))%>% 
  mutate(item = substr(item, 1, str_length(item)-3))
# merge Q1 and Q2
data <- merge(temp1, temp2,by=c("ResponseId","item"))
# create an "s2 answer" variable (same, similar, new)
data <-  data %>%
  mutate(s2_answer = ifelse(s2_q1 == "No", "new", 
                            ifelse(s2_q2 == "Yes", "same", 
                                   ifelse(s2_q2 == "No", "similar", "error"))))
...
data <- data %>% 
  mutate(time = mdy_hm(StartDate), condition = ifelse(hour(time)< 12, "sleep", "wake"))
...
item_list <- item_list %>% mutate(component = ifelse(item_number %in% c(1:64, 129:160),
                                                     "object", "background"))
...
# rearrange
data_joined <- data_joined %>% arrange(ResponseId, item_number) 
...
# mod0 <- lmer(general_recog ~ condition*component+component*valence+condition*valence + (component*valence | ResponseId), data_ind)
# mod1 <- lmer(general_recog ~ condition*component*valence + (component*valence | ResponseId), data_ind)
# anova(mod0, mod1)
...
# linear mixed effect models
# mod2 <- lmer(general_recog ~ condition + valence + (valence | ResponseId), data_ind %>% filter(component == "object"))
# mod3 <- lmer(general_recog ~ condition*valence + (valence | ResponseId), data_ind %>% filter(component == "object"))
# anova(mod2, mod3)
```
...
# mod4 <- lmer(general_recog ~ condition + valence + (valence | ResponseId), data_ind %>% filter(component == "background"))
# mod5 <- lmer(general_recog ~ condition*valence + (valence | ResponseId), data_ind %>% filter(component == "background"))
# anova(mod4, mod5)
```
